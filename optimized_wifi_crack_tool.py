...
class WifiCrackTool:
    ...
    class Crack:
        ...
        def __init__(self,tool:'WifiCrackTool'):
            ...
            self.speed_calculation_window = deque(maxlen=10)  # 用于计算实时速度
            ...
            self.disconnect_interval = 0.05  # 最小断开连接间隔
        ...
        def search_wifi(self):
            ...
            try:
                ...
                for i,data in enumerate(unique_ap_list):#输出扫描到的WiFi名称
                    ssid = data.ssid
        ...
        def crack(self,ssid:str, start_position:int=0):
            ...
            try:
                ...
                with open(self.tool.config_settings_data['pwd_txt_path'],'r', encoding='utf-8', errors='ignore', buffering=buffer_size) as lines:
                    # 根据起始位置跳过前面的行
                    if start_position > 0:
        ...
        def connect(self, ssid, pwd, filetype, count):
            try:
                ...
                self.win.show_msg.send(f"连接尝试失败，密码是{pwd}\n\n", "red")
